...
require "db.php";
$sql = "Select *  from unactivated_user where p_number = '{$k}'";
...
?>
    <tr>    
        ...
        <!-- Adding delete button for each record row -->
            <form action='delete_unactivated_user.php?id="<?php echo $row["p_number"]; ?>"' method="post">
            <button 
            style = "background-color:red;border-color:black; float:right"
            class = "btn"><i class="fa fa-trash"></i
            > Delete</button>
            <input type="hidden" name="d_unactivated_user" value="<?php echo $rows["p_number"]; ?>"  >
